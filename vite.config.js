...
function claudeApiProxyPlugin() {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/chat/claude', async (request, response) => {
        ...
        const safeMessages = chatMessages
          .filter((messageItem) => messageItem && typeof messageItem.text === 'string' && typeof messageItem.role === 'string')
          .slice(-20)
          .map((messageItem) => ({
        ...
        try {
          ...
          if (!anthropicResponse.ok) {
            ...
            response.end(
              JSON.stringify({
                error:
                  anthropicResponseBody?.error?.message ||
                  'Failed to fetch Claude response',
              }),
            )
            return
          }
