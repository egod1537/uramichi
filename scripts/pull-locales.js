...
function resolveCliArguments() {
  ...
  try {
    ...
    const originalArguments = Array.isArray(parsedNpmConfigArgv?.original)
      ? parsedNpmConfigArgv.original
      : [];
    ...
    return originalArguments
      .slice(runScriptIndex + 2)
      .filter((argumentValue) => argumentValue !== '--');
  } catch {
    return [];
  }
}
...
function normalizeConfig(rawConfig) {
  ...
  if (languages.length === 0) {
    throw new Error(
      'locales.config.js is invalid: languages must include at least one language code.',
    );
  }
...
async function processSheet(config, sheetConfig) {
  ...
  try {
    ...
    await writeSheetLocaleFiles(
      config.absoluteOutputDirectory,
      sheetConfig.name,
      sheetData.localeRecordByLanguage,
    );
...
function buildSheetLocaleData(parsedRows, supportedLanguages) {
  ...
  for (let rowIndex = 1; rowIndex < parsedRows.length; rowIndex += 1) {
    ...
    for (const languageCode of supportedLanguages) {
      ...
      localeRecordByLanguage[languageCode][translationKey] = hasTranslation
        ? translationValue
        : translationKey;
