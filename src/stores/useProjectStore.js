...
const createLegacyLineId = (lineCount, legacyIndex) =>
  `line-legacy-${Date.now()}-${lineCount + legacyIndex + 1}`;
...
const initialProjectState = {
  ...
  lines: normalizeLineList(
    rawInitialProjectState.lines || [],
    rawInitialProjectState.measurements || [],
  ),
};
...
const reorderPinsByLayer = (
  ...
  const layerPinIdList = pinList
    .filter((pinItem) => pinItem.layerId === layerId)
    .map((pinItem) => pinItem.id);
  const sourceIndex = layerPinIdList.indexOf(sourcePinId);
  if (sourceIndex < 0) return pinList;
  const targetIndex =
    dropPosition === 'end' ? layerPinIdList.length - 1 : layerPinIdList.indexOf(targetPinId);
  if (targetIndex < 0) return pinList;
  const insertIndex = resolveReorderInsertIndex(
    sourceIndex,
    targetIndex,
    dropPosition,
    layerPinIdList.length,
  );
  const reorderedLayerPinIdList = reorderItemList(layerPinIdList, sourceIndex, insertIndex);
  const pinOrderMap = new Map(
    reorderedLayerPinIdList.map((pinId, pinOrderIndex) => [pinId, pinOrderIndex]),
  );
...
const reorderLinesByLayer = (
  ...
  const layerLineIdList = lineList
    .filter((lineItem) => lineItem.layerId === layerId)
    .map((lineItem) => lineItem.id);
  const sourceIndex = layerLineIdList.indexOf(sourceLineId);
  if (sourceIndex < 0) return lineList;
  const targetIndex =
    dropPosition === 'end' ? layerLineIdList.length - 1 : layerLineIdList.indexOf(targetLineId);
  if (targetIndex < 0) return lineList;
  const insertIndex = resolveReorderInsertIndex(
    sourceIndex,
    targetIndex,
    dropPosition,
    layerLineIdList.length,
  );
  const reorderedLayerLineIdList = reorderItemList(layerLineIdList, sourceIndex, insertIndex);
  const lineOrderMap = new Map(
    reorderedLayerLineIdList.map((lineId, lineOrderIndex) => [lineId, lineOrderIndex]),
  );
...
const reorderLayerObjectsByLayer = (
  pinList,
  lineList,
  layerId,
  sourceObject,
  ...
  const layerObjectList = [
    ...pinList
      .filter((pinItem) => pinItem.layerId === layerId)
      .map((pinItem) => ({ type: 'pin', id: pinItem.id })),
    ...lineList
      .filter((lineItem) => lineItem.layerId === layerId)
      .map((lineItem) => ({ type: 'line', id: lineItem.id })),
  ];
  const sourceIndex = layerObjectList.findIndex(
    (objectItem) => objectItem.type === sourceObject.type && objectItem.id === sourceObject.id,
  );
  if (sourceIndex < 0) {
    return { nextPins: pinList, nextLines: lineList };
  }
  ...
  const targetIndex =
    dropPosition === 'end'
      ...
      : layerObjectList.findIndex(
          (objectItem) =>
            objectItem.type === targetObject.type && objectItem.id === targetObject.id,
        );
  if (targetIndex < 0) {
    return { nextPins: pinList, nextLines: lineList };
  }
  ...
  const layerObjectOrderMap = new Map(
    reorderedLayerObjectList.map((objectItem, objectOrderIndex) => [
      `${objectItem.type}-${objectItem.id}`,
      objectOrderIndex,
    ]),
  );
...
const useProjectStore = create((set) => ({
  ...
  commitSnapshot: (nextSnapshot) =>
    set((state) => {
      ...
      const committedHistory = HistoryManager.commit(
        state.history,
        state.historyIndex,
        normalizedSnapshot,
      );
  ...
  addMarker: (point, pinPatchData = {}) =>
    set((state) => {
      ...
      if (
        !nextActiveLayerId ||
        !nextLayers.some((layerItem) => layerItem.id === nextActiveLayerId)
      ) {
      ...
      const createdPin = createDefaultPinData(
        point,
        nextActiveLayerId,
        state.pins.length,
        pinPatchData,
  ...
  updatePin: (pinId, patchData) =>
    set((state) => {
      const nextPins = state.pins.map((pinItem) =>
        pinItem.id === pinId ? { ...pinItem, ...patchData } : pinItem,
      );
      return {
        pins: nextPins,
        markers: createMarkersFromPins(nextPins),
  ...
  removePin: (pinId) =>
    set((state) => {
      ...
      return {
        ...
        selectedPinIds: state.selectedPinIds.filter(
          (selectedPinIdItem) => selectedPinIdItem !== pinId,
        ),
        history: committedHistory.history,
        historyIndex: committedHistory.historyIndex,
        lastEditedAt: new Date().toISOString(),
  ...
  removePins: (pinIds) =>
    set((state) => {
      ...
      return {
        ...
        selectedPinIds: state.selectedPinIds.filter(
          (selectedPinIdItem) => !pinIdSetToRemove.has(selectedPinIdItem),
        ),
        history: committedHistory.history,
        historyIndex: committedHistory.historyIndex,
        lastEditedAt: new Date().toISOString(),
  ...
  reorderLayers: (sourceLayerId, targetLayerId, dropPosition = 'before') =>
    set((state) => {
      ...
      const sourceLayerIndex = state.layers.findIndex(
        (layerItem) => layerItem.id === sourceLayerId,
      );
      if (sourceLayerIndex < 0) return state;
      const targetLayerIndex =
        dropPosition === 'end'
          ? state.layers.length - 1
          : state.layers.findIndex((layerItem) => layerItem.id === targetLayerId);
  ...
  reorderPinsInLayer: (layerId, sourcePinId, targetPinId, dropPosition = 'before') =>
    set((state) => {
      const nextPins = reorderPinsByLayer(
        state.pins,
        layerId,
        sourcePinId,
        targetPinId,
  ...
  reorderLinesInLayer: (layerId, sourceLineId, targetLineId, dropPosition = 'before') =>
    set((state) => {
      const nextLines = reorderLinesByLayer(
        state.lines,
        layerId,
        sourceLineId,
        targetLineId,
    ...
    set((state) => {
      ...
      if (!resolvedLayerId || !nextLayers.some((layerItem) => layerItem.id === resolvedLayerId)) {
        if (!nextLayers.length) {
          ...
          resolvedLayerId =
            state.activeLayerId &&
            nextLayers.some((layerItem) => layerItem.id === state.activeLayerId)
              ? state.activeLayerId
              : nextLayers[0].id;
        }
      }
      ...
      const normalizedLineData = {
        ...
        name:
          lineData.name ||
          (normalizedShapeType === 'polygon'
            ? `도형 ${state.lines.length + 1}`
            : `선 ${state.lines.length + 1}`),
  ...
  updateLine: (lineId, patchData) =>
    set((state) => ({
      lines: state.lines.map((lineItem) =>
        lineItem.id === lineId ? { ...lineItem, ...patchData } : lineItem,
      ),
      lastEditedAt: new Date().toISOString(),
    })),
  removeLine: (lineId) =>
  ...
  removeLayer: (layerId) =>
    set((state) => {
      ...
      const removedPinIdSet = new Set(
        state.pins.filter((pinItem) => pinItem.layerId === layerId).map((pinItem) => pinItem.id),
      );
      const nextPins = state.pins.filter((pinItem) => pinItem.layerId !== layerId);
      const removedLineIdSet = new Set(
        state.lines
          .filter((lineItem) => lineItem.layerId === layerId)
          .map((lineItem) => lineItem.id),
      );
      ...
      const nextActiveLayerId =
        state.activeLayerId === layerId ? (nextLayers[0]?.id ?? null) : state.activeLayerId;
      const committedHistory = HistoryManager.commit(state.history, state.historyIndex, {
        ...createSnapshotFromState(state),
        markers: createMarkersFromPins(nextPins),
        lines: nextLines,
      ...
      return {
        ...
        selectedPinIds: state.selectedPinIds.filter(
          (selectedPinIdItem) => !removedPinIdSet.has(selectedPinIdItem),
        ),
  ...
  renameLayer: (layerId, layerName) =>
    set((state) => ({
      layers: state.layers.map((layerItem) =>
