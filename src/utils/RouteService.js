...
class RouteService {
  ...
  static createRouteCacheData(routeResult, routePath) {
    ...
    const firstTransitStep = firstLeg?.steps?.find(
      (stepItem) => stepItem.travel_mode === 'TRANSIT',
    );
    const transitLineName =
      firstTransitStep?.transit?.line?.short_name ||
      firstTransitStep?.transit?.line?.name ||
      firstTransitStep?.instructions ||
      '';
  ...
  static async createRouteEntityOrNull({ start, end, travelMode, currentRoutes, activeLayerId }) {
    ...
    const buildRouteEntity = (routeData) => {
      ...
      return RouteService.createRouteEntity(
        routeId,
        routeData,
        start,
        end,
    ...
    return new Promise((resolve) => {
      ...
      directionsService.route(
        RouteService.getRouteRequest(start, end, travelMode),
        (result, status) => {
          ...
          const normalizedPath = overviewPath.map((locationPoint) => ({
