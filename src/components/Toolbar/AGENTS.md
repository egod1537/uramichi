...
[codex] 2026-02-28 툴바 필터 버튼/우측 패널 통합 메모
- `Toolbar.jsx` 하단 버튼 행 우측에 필터 버튼을 추가하고, 같은 라인에 현재 적용된 필터 요약(시간 범위 + 아이콘 필터 개수)을 표시함.
- 필터 버튼 클릭 시 우측 오버레이 패널이 열리며 시간 범위 슬라이더와 핀 아이콘 상세 필터를 조정할 수 있도록 연결함.
- 시간 슬라이더 UI는 기존 지도 시간 필터의 양방향 range 패턴(30분 스텝, 두 핸들 z-index 제어)을 재사용함.
