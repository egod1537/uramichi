...
[codex] 2026-02-28 시간 필터 구간 드래그 동기 이동/인터랙션 색상 메모
- `MapOverlays.jsx` 시간 필터에서 오렌지 구간(bar)을 직접 드래그하면 시작/종료 슬라이더가 동일 간격을 유지한 채 같은 방향으로 함께 이동하도록 포인터 드래그 상태를 추가함.
- 구간 드래그 또는 개별 핸들 조작이 한 번이라도 발생하면 활성 구간 색을 기존보다 진하게 유지하고, 드래그 중에는 더 진한 색으로 바꿔 인터랙션 발생 여부를 시각적으로 구분하도록 반영함.
