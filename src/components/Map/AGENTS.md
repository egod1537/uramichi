...
[codex] 2026-02-28 PinPopup 헤더 UI/팝오버 구조 변경 메모
- `PinPopup.jsx` 팝업 앵커 오프셋을 `+28px`로 늘려 핀 기준점보다 더 위에 보이도록 조정함.
- 기존 `편집` 토글 버튼을 제거하고, 카테고리/체류시간/비용 편집 블록은 기본 노출 상태로 전환함.
- 핀 아이콘 버튼 클릭 시 본문 아래 인라인이 아닌 버튼 상단(`bottom-full`)에 별도 아이콘 팝오버가 열리도록 변경함.
- 삭제 버튼 왼쪽에 현재 색상 스와치 버튼을 추가하고, 클릭 시 별도 컬러 팔레트 팝오버가 열려 색상을 선택하도록 변경함.
[codex] 2026-02-28 맵 하단 필터 제거 및 스토어 필터 적용 메모
- `MapOverlays.jsx`에서 하단 시간/핀 필터 바를 제거하고, 경로 이동수단 선택/경로 요약 오버레이만 유지함.
- `Map.jsx`의 `getVisiblePins`에 시간 필터(`timeFilterRange`)와 아이콘 필터(`pinIconFilters`)를 함께 반영해 실제 핀 렌더링 결과에 적용되도록 수정함.
- 시간 필터는 핀 `openingHours`를 정규화한 뒤 겹치는 구간이 있는 핀만 통과하도록 처리하고, 영업시간 데이터가 없는 핀은 기본 노출로 유지함.
