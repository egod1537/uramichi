...
function MapOverlays({
  ...
  const activePinFilterItems = ICON_FILTER_OPTIONS.filter((filterItem) =>
    pinIconFilters.includes(filterItem.key),
  );
  const collapsedPreviewFilterItems = activePinFilterItems.slice(0, 2);
  const hiddenPreviewFilterCount = Math.max(
    activePinFilterItems.length - collapsedPreviewFilterItems.length,
    0,
  );
  const startMinutes = normalizeSliderMinutes(
    convertTimeStringToMinutes(timeFilterRange.start),
    9 * 60,
  );
  const endMinutes = normalizeSliderMinutes(
    convertTimeStringToMinutes(timeFilterRange.end),
    18 * 60,
  );
  ...
  useEffect(() => {
    ...
    const handleWindowPointerMove = (event) => {
      ...
      const minuteDeltaRaw =
        (moveDistancePixels / dragState.trackWidthPixels) * TIME_SLIDER_MINUTES_MAX;
      const minuteDeltaRounded =
        Math.round(minuteDeltaRaw / TIME_SLIDER_MINUTES_STEP) * TIME_SLIDER_MINUTES_STEP;
      if (!minuteDeltaRounded) return;

      const nextStartMinutes = Math.max(
        0,
        Math.min(dragState.maxStartMinutes, dragState.initialStartMinutes + minuteDeltaRounded),
      );
  ...
  const handleStartSliderChange = (nextStartMinutesText) => {
    const nextStartMinutes = normalizeSliderMinutes(
      ...
      <div className="absolute bottom-4 left-1/2 z-20 flex -translate-x-1/2 items-end gap-2">
        <div
          className={`rounded-xl border border-gray-200 bg-white/95 px-3 py-2 shadow-lg transition-all duration-200 ${
            isTimeFilterExpanded
              ? 'inline-flex max-w-[58vw] items-center gap-3'
              : 'inline-flex w-auto items-center gap-2'
          }`}
        >
          <img
            src="/svg/map-time-filter.svg"
            alt="지도 시간 필터"
              ...
              {activePinFilterItems.length > 0 && (
                <div className="flex items-center gap-1">
                  {collapsedPreviewFilterItems.map((filterItem) => (
                    ...
                    >
                      <img
                        src={filterItem.svgPath}
                        alt={filterItem.label}
                        className="h-3.5 w-3.5"
                      />
                    </span>
                  ))}
                  {hiddenPreviewFilterCount > 0 && (
                    <span className="text-xs font-medium text-blue-700">
                      +{hiddenPreviewFilterCount}
                    </span>
                  )}
                </div>
              )}
      ...
      {currentMode === TOOL_MODES.ADD_ROUTE && (
        <div className="absolute left-1/2 top-4 z-20 flex -translate-x-1/2 items-center gap-2 rounded-md bg-white px-3 py-2 text-sm shadow">
          ...
          <span className="text-gray-700">
            {routeDraft.start ? '도착점을 클릭해 경로를 완성하세요' : '출발점을 클릭하세요'}
          </span>
        </div>
      )}
