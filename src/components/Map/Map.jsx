...
const lineColorSequence = [
  COLOR_PRESETS.primaryBlue,
  COLOR_PRESETS.routeGreen,
  COLOR_PRESETS.measureOrange,
  '#8b5cf6',
];
...
class Map extends React.Component {
  ...
  componentDidUpdate(previousProps) {
    ...
    if (
      selectedPin &&
      (!previousSelectedPin || previousSelectedPin.id !== selectedPin.id) &&
      this.mapInstance
    ) {
    ...
    if (
      previousStore?.poiSearchRequest !== currentStore.poiSearchRequest &&
      currentStore.poiSearchRequest
    ) {
  ...
  getVisibleLayerIdSet = (projectStore) =>
    new Set(
      projectStore.layers.filter((layerItem) => layerItem.visible).map((layerItem) => layerItem.id),
    );

  getVisiblePins = (projectStore) => {
    const visibleLayerIdSet = this.getVisibleLayerIdSet(projectStore);
    const layerVisiblePins = projectStore.pins.filter((pinItem) =>
      visibleLayerIdSet.has(pinItem.layerId),
    );
    if (!projectStore.pinIconFilters.length) return layerVisiblePins;
    const activeIconSet = new Set(
      ICON_FILTER_OPTIONS.filter((filterItem) =>
        projectStore.pinIconFilters.includes(filterItem.key),
      ).map((filterItem) => filterItem.key),
    );
    return layerVisiblePins.filter((pinItem) =>
      activeIconSet.has(getTravelPinIconKey(pinItem.icon)),
    );
  };
  ...
  getSelectedPin = (projectStore) =>
    this.getVisiblePins(projectStore).find(
      (pinItem) => pinItem.id === projectStore.selectedPinId,
    ) || null;

  getSelectedLine = (projectStore) =>
    this.getVisibleLines(projectStore).find(
      (lineItem) => lineItem.id === projectStore.selectedLineId,
    ) || null;
  ...
  getDistanceMeterByPixel = (referenceLat) => {
    ...
    const metersPerPixel =
      (156543.03392 * Math.cos((referenceLat * Math.PI) / 180)) / Math.pow(2, zoom);
  ...
  createLineEntity = (linePoints) => {
    ...
    const isClosed =
      linePoints.length >= 3 && Math.hypot(latGapMeter, lngGapMeter) <= closeDistanceMeters;
    const normalizedPoints = isClosed ? [...linePoints.slice(0, -1), firstPoint] : linePoints;
    const shapeType = isClosed ? 'polygon' : 'line';
    const lineNumber =
      projectStore.lines.filter((lineItem) => lineItem.shapeType !== 'polygon').length + 1;
    const polygonNumber =
      projectStore.lines.filter((lineItem) => lineItem.shapeType === 'polygon').length + 1;
  ...
  requestPoiDetail = (placeId, position, fallbackData = {}) => {
    ...
    this.setState({
      selectedPoiDetail: createLoadingPoiDetail(placeId, position),
      poiDetailStatus: 'loading',
    });
    ...
    placeServiceInstance.getDetails(
      ...
      (placeResult, placeStatus) => {
        if (placeStatus !== window.google.maps.places.PlacesServiceStatus.OK || !placeResult) {
          this.setState({
            selectedPoiDetail: createErrorPoiDetail(placeId, position, fallbackData),
            poiDetailStatus: 'error',
          });
          return;
        }
        this.setState({
  ...
  createModeEventContext = (event) => {
    ...
    const longitude = event?.latLng?.lng();
    const clickedPoint =
      latitude === undefined || longitude === undefined ? null : { lat: latitude, lng: longitude };
  ...
  updateLinePointByIndex = (lineId, linePointIndex, event) => {
    ...
    const snapCandidates = [
      ...projectStore.pins.map((pinItem) => pinItem.position),
      ...projectStore.lines.flatMap((lineItem) => lineItem.points),
    ];
  ...
  handleDraftLinePointDrag = (linePointIndex, event) => {
    ...
    const snapCandidates = this.getLineSnapPointList().filter(
      (pointItem, pointIndex) => pointIndex !== linePointIndex,
    );
    const snappedResult = this.getSnappedPoint(draggedPoint, snapCandidates);
    this.setState((previousState) => ({
      linePath: previousState.linePath.map((linePoint, currentIndex) =>
        currentIndex === linePointIndex ? snappedResult.point : linePoint,
      ),
      snapTargetPoint: snappedResult.isSnapped ? snappedResult.point : null,
    }));
  };
  ...
  render() {
    ...
    const selectedLine = this.getSelectedLine(projectStore);
    const previewLinePath =
      previewLinePoint && linePath.length ? [...linePath, previewLinePoint] : [];
      ...
      <>
        <GoogleMap
          ...
          options={{
            ...MAP_OPTIONS,
            clickableIcons:
              projectStore.currentMode !== TOOL_MODES.ADD_MARKER &&
              projectStore.currentMode !== TOOL_MODES.DRAW_LINE,
            draggableCursor:
              projectStore.currentMode === TOOL_MODES.DRAW_LINE ? 'crosshair' : undefined,
          }}
        >
          <PinLayer
            pins={visiblePins}
          ...
          <PoiDetailOverlay
            poiDetail={selectedPoiDetail}
            onClose={this.clearPoiDetail}
            onAddPoiToMap={this.handleAddPoiToMap}
          />
        ...
        <MapOverlays
          ...
          onSetTimeFilterExpanded={(nextValue) =>
            this.setState({ isTimeFilterExpanded: nextValue })
          }
