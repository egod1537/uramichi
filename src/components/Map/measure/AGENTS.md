...
[codex] 2026-02-27 거리 측정 종료 시 드래그 상태 정리 메모
- `useDistanceMeasureInteraction`의 기본/우클릭/ESC 종료 경로에서 `setDraggingMeasurePointIndex(null)`을 공통으로 호출해 종료 후 점 드래그 상태가 남지 않도록 정리함.
