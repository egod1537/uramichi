...
[codex] 2026-02-27 거리 라벨 박스 레이아웃 보정 메모
- `src/components/Map/measure/MeasureLabels.jsx`의 구간/총 거리 라벨 패딩과 폰트 크기를 키워 작은 배경에서 텍스트가 튀어나오던 문제를 완화함.
- 구간 라벨은 흰 배경 고정(`bg-white`) + `text-sm`로, 총 거리 라벨은 동일 크기 규칙으로 맞춰 두 라벨의 시각 밀도를 통일함.
