...
const resolveMeasurementShapeType = (measurePointPath) => {
  ...
  const [firstPoint] = measurePointPath;
  const lastPoint = measurePointPath.at(-1);
...
const createTotalLabelData = (measurePointPath) => {
  ...
  return {
    id: 'measure-total',
    position: terminalPoint,
    label: `총 ${formatDistanceLabel(totalDistanceInMeters)}`,
  };
};
...
function useMeasureInteraction({
  ...
  const measureSegmentLabelDataList = useMemo(
    () => createSegmentLabelDataList(measurePath),
    [measurePath],
  );
  ...
  const previewMeasurePath = useMemo(() => {
    if (currentMode !== TOOL_MODES.MEASURE_DISTANCE && currentMode !== TOOL_MODES.DRAW_LINE)
      return [];
  ...
  const handleMeasurePointDragStart = useCallback(
    (pointIndex) => {
