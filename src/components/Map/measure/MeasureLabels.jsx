    ...
    <>
      {measureSegmentLabelDataList.map((segmentLabelData) => (
        <OverlayView key={segmentLabelData.id} position={segmentLabelData.position} mapPaneName={measureOverlayPane}>
          <div className="-translate-x-1/2 -translate-y-[calc(100%+10px)] whitespace-nowrap rounded-md border border-gray-200 bg-white px-3 py-2 text-sm font-semibold leading-none text-gray-700 shadow">
      ...
      {measureTotalLabelData ? (
        <OverlayView key={measureTotalLabelData.id} position={measureTotalLabelData.position} mapPaneName={measureOverlayPane}>
          <div className="-translate-x-1/2 -translate-y-[calc(100%+18px)] whitespace-nowrap rounded-md border border-orange-500 bg-[#f97316] px-3 py-2 text-sm font-semibold leading-none text-white shadow">
