...
[codex] 2026-02-28 레이어 패널 선 점수 표기 제거 메모
- `LayerRow.jsx`에서 선(line) 오브젝트 하단에 표시하던 `N점` 보조 텍스트를 제거해 레이어 패널에서 점 개수가 보이지 않도록 변경함.
[codex] 2026-02-28 레이어 패널 도형 아이콘 표기 메모
- `LayerRow.jsx`의 line 오브젝트 아이콘에서 `shapeType === polygon`을 `🔷`로 표기해 선(`🧵`)과 도형을 구분해 표시함.

[codex] 2026-02-28 레이어 패널 핀/선 교차 드래그 정렬 메모
- `LayerRow.jsx` 오브젝트 드롭 처리에서 타입 일치 제한을 제거하고 `reorderLayerObjectsInLayer`를 호출하도록 변경함.
- 이제 같은 레이어에서 핀과 선이 하나의 목록 순서로 섞여 이동되며, `before/after/end` 드롭 위치가 통합 순서에 반영됨.
[codex] 2026-02-28 레이어 패널 필터 팝업 전환 메모
- `LayerPanel.jsx`의 기존 인라인 아이콘 필터 버튼 행을 제거하고, `필터` 버튼 + 적용 요약 텍스트 1줄 구조로 변경함.
- 필터 버튼 클릭 시 우측 팝업에서 아이콘 필터와 시간 필터(체크박스 + 듀얼 슬라이더)를 설정하도록 구성함.
- 시간 필터 UI는 지도 오버레이 시간 필터의 슬라이더 동작(30분 단위, 시작/종료 핸들) 로직을 재사용해 일관된 UX를 유지함.
- 레이어 패널 핀 목록 필터링은 아이콘 조건과 시간 조건을 함께 적용하며, 시간 필터는 핀 `openingHours`와 겹치는 시간대만 통과하도록 처리함.
